...
pub fn iter_sort_jc_adaptive<T: Copy + Ord + Send + Sync>(input: &mut [T], num_threads: u32) {
    ...
    (input, buffer.as_mut_slice())
        ...
        .join_context_policy(std::cmp::min(
            (2.0 * (num_threads as f32).log2().ceil() - (num_threads as f32).log2().floor()) as u32,
            5u32,
        ))
        .even_levels()
...
pub fn iter_sort_bd_adaptive<T: Copy + Ord + Send + Sync>(input: &mut [T], num_threads: u32) {
    let input_len = input.len();
    let mut buffer: Vec<T> = Vec::with_capacity(input_len);
    unsafe {
        buffer.set_len(input_len);
...
pub fn iter_sort_rayon_adaptive<T: Copy + Ord + Send + Sync>(input: &mut [T], num_threads: u32) {
    let input_len = input.len();
    let mut buffer: Vec<T> = Vec::with_capacity(input_len);
    unsafe {
        buffer.set_len(input_len);
    ...
    (input, buffer.as_mut_slice())
        ...
        .rayon(
            (2.0 * (num_threads as f32).log2().ceil() - (num_threads as f32).log2().floor())
                as usize,
        )
        .even_levels()
        .reduce_with(|(left_input, left_output), (right_input, right_output)| {
