...
use crate::prelude::*;
use crate::{adaptive_jp_slice_merge, jc_slice_merge};
#[cfg(feature = "logs")]
...
pub fn slice_sort_jc_jc<T: Copy + Ord + Send + Sync>(input: &mut [T]) {
    let input_len = input.len();
    let mut buffer: Vec<T> = Vec::with_capacity(input_len);
    unsafe {
        buffer.set_len(input_len);
...
pub fn slice_sort_jc_adaptive_jp<T: Copy + Ord + Send + Sync>(input: &mut [T]) {
    let input_len = input.len();
    let mut buffer: Vec<T> = Vec::with_capacity(input_len);
    unsafe {
        buffer.set_len(input_len);
